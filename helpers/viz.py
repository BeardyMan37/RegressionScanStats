...
def plot_all_spectra(
    ...
    # viz/IO
    buffer:   int = 0,
    out_dir:  str = "images/all_spectra",
    data_dir: str = "data/all_spectra",
    # predictions
    sra_preds:         List[np.ndarray | None] | None = None,
    sri_idxs_unmasked: List[np.ndarray | None] | None = None,
    sri_vals_unmasked: List[np.ndarray | None] | None = None,
    sr_factor: int = 1,
    fixed_bins_nat: List[int] | None = None,
    # ranking (for parquet output only — does not affect plot order)
    rank_by: str = "unmasked",
) -> None:
    os.makedirs(out_dir,  exist_ok=True)
    os.makedirs(data_dir, exist_ok=True)
    ...
    # ------------------------------------------------------------------
    # Build and save ranked summary parquet  (from plot_top_k verbatim)
    # ------------------------------------------------------------------
    score_map_all = {
        "masked":   np.asarray(scores_masked,   dtype=float),
        "unmasked": np.asarray(scores_unmasked, dtype=float),
    ...
    def _mk_map(vec):
        return dict(zip(top_uids, np.asarray(vec, dtype=object)[order_desc]))
    ...
    out_file = os.path.join(data_dir, f"output_length_{actual_spec_arrays.shape[1]}.parquet")
    sub_df.to_parquet(out_file, engine="pyarrow", compression="zstd")
    logging.info("Wrote summary parquet: %s", out_file)
 
    # ------------------------------------------------------------------
    # Per-spectrum plots (unsorted, unmasked window + SRA/SRI only)
    # ------------------------------------------------------------------
    # uids   = np.asarray(meta["uid"])
    # n_rows = actual_spec_arrays.shape[0]
 
    # def _plot_sri(ax, spec, freq, i0):
    #     if sri_idxs_unmasked is None or sri_vals_unmasked is None:
    #         return
    #     if i0 >= len(sri_idxs_unmasked) or sri_idxs_unmasked[i0] is None or sri_vals_unmasked[i0] is None:
    #         return
    #     idxs = np.asarray(sri_idxs_unmasked[i0], dtype=int)
    #     vals = np.asarray(sri_vals_unmasked[i0], dtype=float)
    #     if len(idxs) == 2 and idxs[0] == 0 and idxs[1] == 0:
    #         return
    #     sri_full = np.full(len(spec), np.nan)
    #     for p, v in zip(idxs * sr_factor, vals):
    #         p_end = min(int(p) + sr_factor, len(spec))
    #         sri_full[int(p):p_end] = v
    #     ax.plot(freq, sri_full, ".", ms=2, color="C5", label="SRI")
 
    # for i in range(n_rows):
    #     spec  = actual_spec_arrays[i]
    #     freq  = np.asarray(freqs[i], dtype=float)
    #     n_ch  = len(freq)
    #     uid   = str(uids[i])
    #     a_u, b_u = windows_unmasked[i]
    #     score    = scores_unmasked[i]
 
    #     fig, ax = plt.subplots(figsize=(10, 3))
 
    #     # raw spectrum
    #     ax.plot(freq, spec, color="C0", linewidth=0.8, label="Actual")
 
    #     # unmasked window
    #     au0, au1 = _clamp_pair(a_u, b_u, n_ch)
    #     if au0 < au1:
    #         ax.axvspan(float(freq[au0]), float(freq[au1]),
    #                    color="C2", alpha=0.25, label="Window")
 
    #     # SRA predicted curve
    #     if (sra_preds is not None
    #             and i < len(sra_preds)
    #             and sra_preds[i] is not None
    #             and len(sra_preds[i]) > 0):
    #         pred_full = np.full(len(spec), np.nan)
    #         sra_up    = np.repeat(sra_preds[i], sr_factor)
    #         end       = len(spec) - buf
    #         pred_full[buf:end] = sra_up[: max(0, end - buf)]
    #         ax.plot(freq, pred_full, ".", ms=2, color="C4", label="SRA pred")
 
    #     # SRI overlay
    #     _plot_sri(ax, spec, freq, i)
 
    #     # title
    #     ax.set_title(
    #         f"UID: {uid}   score={score:.4f}   window=[{a_u}, {b_u}]",
    #         fontsize=8,
    #     )
    ...
    #     # legend
    #     legend_elements = [
    #         Line2D([0], [0], color="C0", linewidth=0.8,                label="Actual"),
    #         Line2D([0], [0], marker=".", linestyle="None", color="C4", label="SRA pred"),
    #         Line2D([0], [0], marker=".", linestyle="None", color="C5", label="SRI"),
    #         Patch(facecolor="C2", alpha=0.25,                          label="Window"),
    #     ]
    ...
    #     fig.savefig(os.path.join(out_dir, f"{safe_uid}.png"), dpi=150, bbox_inches="tight")
    #     plt.close(fig)